...
if __name__ == "__main__":
    ...
    burgers_problem.discretise_domain(
        n=200, mode="grid", variables="t", locations=["D"]
    )
    burgers_problem.discretise_domain(
        n=20, mode="grid", variables="x", locations=["D"]
    )
    burgers_problem.discretise_domain(
        n=150, mode="random", locations=["gamma1", "gamma2", "t0"]
    )
    ...
    if args.load:
        pinn = PINN.load_from_checkpoint(
            checkpoint_path=args.load, problem=burgers_problem, model=model
        )
        plotter = Plotter()
        plotter.plot(pinn)
    else:
