...
if __name__ == "__main__":
    ...
    wave_problem = Wave()
    wave_problem.discretise_domain(
        1000,
        "random",
        locations=["D", "t0", "gamma1", "gamma2", "gamma3", "gamma4"],
    )
    ...
    # create solver
    pinn = PINN(
        problem=wave_problem, model=model, optimizer_kwargs={"lr": 0.006}
    )
    ...
    if args.load:
        pinn = PINN.load_from_checkpoint(
            checkpoint_path=args.load, problem=wave_problem, model=model
        )
        plotter = Plotter()
        plotter.plot(pinn)
    else:
