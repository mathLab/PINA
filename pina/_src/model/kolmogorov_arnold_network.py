    ...
    def enable_sparsification(self, threshold: float = 1e-4):
        ...
        with torch.no_grad():
            for layer in self.kan_layers:
                ...
                layer.scale_base.data[
                    torch.abs(layer.scale_base.data) < threshold
                ] = 0
                layer.scale_spline.data[
                    torch.abs(layer.scale_spline.data) < threshold
                ] = 0

                # Update mask
                layer.mask.data = (
                    (torch.abs(layer.scale_base) >= threshold)
                    | (torch.abs(layer.scale_spline) >= threshold)
                ).float()
