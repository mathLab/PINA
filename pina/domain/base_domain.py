...
class BaseDomain(DomainInterface, metaclass=ABCMeta):
    ...
    def __init__(self, domain_dict=None):
        ...
        if not isinstance(domain_dict, dict):
            raise TypeError(
                "domain_dict must be dict: {name: number | (low, high)}"
            )

        # Check domain_dict is not empty
        if not domain_dict:
            raise ValueError(
        ...
        # Initialize sampling modes with all available modes
        self.sample_modes = self.__AVAILABLE_MODES
    ...
    def sample_modes(self, values):
        ...
        # Validate modes
        invalid = [v for v in values if v not in BaseDomain.__AVAILABLE_MODES]
        if invalid:
            raise ValueError(
                f"Invalid sampling mode(s): {invalid}. "
                f"Available: {list(BaseDomain.__AVAILABLE_MODES)}"
            )

        # Store modes
        self._sample_modes = values
