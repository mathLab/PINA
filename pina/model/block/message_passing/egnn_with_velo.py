...
class EnEquivariantNetworkBlock(MessagePassing):
    ...
    def forward(self, x, pos, edge_index, edge_attr=None, velo=None):
        """
        ...
        if velo is not None:
            ...
            edge_index=edge_index, x=x, pos=pos, edge_attr=edge_attr, velo=velo
        )
    ...
    def update(self, aggregated_inputs, x, pos, edge_index, velo=None):
        """
        ...
        if velo is not None:
            # Velo net
            # the multplication broads casts to a element wise multiplication
            v = self.update_velo_net(x) * velo + agg_message/c
            pos = pos + v
        else:
