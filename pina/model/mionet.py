...
class MIONet(torch.nn.Module):
    ...
    MIONet is a general architecture for learning Operators defined on the
    tensor product of Banach spaces. Unlike traditional machine learning methods
    MIONet is designed to map entire functions to other functions.
    ...
    def __init__(
        ...
        :param dict networks: The neural networks to use as models. The ``dict``
            ...
            network. If a list of ``int`` is passed, the corresponding columns
        ...
        :Example:
            ...
            >>> ...     branch_net2 : ['x', 'y'],
            >>> ...     trunk_net : ['z']}
            >>> model = MIONet(networks=nets, reduction='+', aggregator='*')
            >>> model
            MIONet(
            (models): ModuleList(
                (0): FeedForward(
    ...
    def _init_aggregator(self, aggregator):
        ...
        elif isinstance(aggregator, torch.nn.Module) or is_function(aggregator):
