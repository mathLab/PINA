...
from pina.domain import Union, EllipsoidDomain, CartesianDomain, SimplexDomain
...
def test_is_inside(check_border):
    ...
    pt_out = LabelTensor(torch.tensor([[-5, 0]]), ["x", "y"])
    pt_border = LabelTensor(torch.tensor([[-1, 0]]), ["x", "y"])

    # Union
    union = Union(__geometries[0])
    ...
    for pt, exp in zip([pt_in, pt_out, pt_border], truth):
        assert union.is_inside(pt, check_border=check_border) == exp

    # Should fail if point is not a LabelTensor
    with pytest.raises(ValueError):
        union.is_inside(torch.Tensor([0.5, 0.5]), check_border=check_border)

    # Should fail if the labels of the point differ from the domain
    with pytest.raises(ValueError):
        pt = LabelTensor(torch.Tensor([0.5, 0.5]), ["a", "b"])
        union.is_inside(pt, check_border=check_border)
...
def test_update(domain_class):
    ...
    domain_2 = domain_class({"x": [1, 2], "y": [0, 2]})
    union = Union([domain_1, domain_2])

    # Update the union with another valid domain
    domain_3 = domain_class({"t": [0, 1], "w": 0})
    updated_union = union.update(domain_3)

    # Check that the union has been updated correctly
    assert len(updated_union.geometries) == 2
    assert updated_union.variables == sorted(["x", "y", "t", "w"])
    for i, g in enumerate(updated_union.geometries):
        assert g._range == {**union.geometries[i]._range, **domain_3._range}
        assert g._fixed == {**union.geometries[i]._fixed, **domain_3._fixed}

    # Should fail if trying to update the union of different geometry types
    with pytest.raises(NotImplementedError):
        union = Union(__geometries[0])
        union.update(simplex_1)
