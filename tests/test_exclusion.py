...
def test_constructor_three_CartesianDomains():
    Exclusion([CartesianDomain({'x': [0, 2], 'y': [0, 2]}), CartesianDomain({'x': [1, 3], 'y': [1, 3]}), CartesianDomain({'x': [2, 4], 'y': [2, 4]})])
...
def test_sample():
    n = 100
    domain = Exclusion([EllipsoidDomain({'x': [-1, 1], 'y': [-1, 1]}), CartesianDomain({'x': [-0.5, 0.5], 'y': [-0.5, 0.5]})])
    pts = domain.sample(n)
