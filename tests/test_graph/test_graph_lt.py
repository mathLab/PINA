...
def test_build_multi_graph_single_val(pos):
    x = LabelTensor(torch.rand(10, 2), ["u", "v"])
    graph = RadiusGraph(x=x, pos=pos, build_edge_attr=False, r=0.3)
    assert len(graph.data) == 3
    data = graph.data
...
@pytest.mark.parametrize(
    "additional_parameters",
    [
        ({"y": LabelTensor(torch.rand(4, 10, 1), ["y"])}),
        ({"y": [LabelTensor(torch.rand(10, 1), ["y"]) for _ in range(4)]}),
    ],
)
def test_wrong_additional_parameters(additional_parameters):
    x = LabelTensor(torch.rand(3, 10, 2), ["u", "v"])
    pos = LabelTensor(torch.rand(3, 10, 3), ["x", "y", "z"])
