...
model = FeedForward(n_feats, n_feats, 32, 2)
...
def test_constructor(use_lt, bool_value, eps, aggregation_strategy):
    ...
    # Should fail if aggregation_strategy is not a callable function
    with pytest.raises(ValueError):
        AutoregressiveSolver(
            problem=problem,
            model=model,
            ...
            use_lt=use_lt,
            eps=eps,
            aggregation_strategy="not_a_function",
        )
...
def test_solver_train(use_lt, batch_size, compile):
    ...
    # Assert that the model is compiled if compile is True
    if trainer.compile:
...
@pytest.mark.parametrize("batch_size", [None, 1, 2, 5])
@pytest.mark.parametrize("compile", [True, False])
def test_solver_validation(use_lt, batch_size, compile):
...
@pytest.mark.parametrize("batch_size", [None, 1, 2, 5])
@pytest.mark.parametrize("compile", [True, False])
def test_solver_test(use_lt, batch_size, compile):
