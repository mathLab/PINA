...
def test_constructor(problem, eps):
    with pytest.raises(ValueError):
        CausalPINN(problem=DummySpatialProblem(), model=model)
    causal_pinn = CausalPINN(problem=problem, model=model, eps=eps)
...
def test_pinn_train(problem, batch_size):
    causal_pinn = CausalPINN(problem=problem, model=model)
