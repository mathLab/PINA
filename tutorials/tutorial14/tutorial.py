...
#
# The final ensemble is simply a **list of PyTorch models**, which we will later pass to the `PhysicsInformedEnsembleSolver`.
...
# Let's visualize the networks output before training
...
#
# Once the `PhysicsInformedEnsembleSolver` solver is defined with all the models, we train them using the `Trainer` class, as with any other solver in **PINA**. We also build a callback to store the value of `u(0.5)` during training iterations.
...
class StoreValue(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        for model in pl_module.models:
            output = model(input_).tensor.flatten().detach().cpu()
            epoch_outputs.append(output)

        epoch_outputs = torch.stack(epoch_outputs, dim=0)
        self.store.append(epoch_outputs)
