...
# You can define as many conditions as needed, and the model will attempt to minimize all of them simultaneously! You can access the input and target data for a specified condition as follows: `problem.conditions['<condition name>'].input`, `problem.conditions['<condition name>'].target`.
# 
...
# This is `False` since no domain has been discretized. One can check which domains have been discretized calling `problem.discretised_domains`:

# In[ ]:


problem.discretised_domains


# To discretise the domains in the problem you can use the `discretise_domain` method:
...
# The points are saved in a python `dict`, and can be accessed by calling the `discretised_domains` of the problem.
